...
const signeasyintegration_response_get_ListPendingEnvelopesSlice = createSlice({
  ...
  reducers: {},
  extraReducers: {
    [signeasyintegration_get_rs_embedded_read.pending]: (state, action) => {
      if (state.api.loading === "idle") {
        state.api.loading = "pending"
      }
    },
    [signeasyintegration_get_rs_embedded_read.fulfilled]: (state, action) => {
    ...
    [signeasyintegration_get_rs_embedded_read.rejected]: (state, action) => {
      if (state.api.loading === "pending") {
